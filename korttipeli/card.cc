...
void Card::setupFront()
{
    ...
    font.setPixelSize(SUIT_PIXMAP_SIZE.height());
    painter.setFont(font);
    painter.drawPixmap(10, 10,
                       suitPixmap_, 0, 0, 0, 0);
    painter.drawPixmap(frontPixmap.width()-suitPixmap_.width()-10,
                       frontPixmap.height()-suitPixmap_.height()-10,
                       suitPixmap_, 0, 0, 0, 0);
    ...
    case 14:
        ...
        break;
    default:
        cardValue = QString::number(value_);
    }

    painter.drawText(frontPixmap.width()/2-suitPixmap_.width()/2,
                     frontPixmap.height()/2-suitPixmap_.height()/2,
                     suitPixmap_.width()+5, suitPixmap_.height() + 5,
                     Qt::AlignCenter, QString(cardValue));
